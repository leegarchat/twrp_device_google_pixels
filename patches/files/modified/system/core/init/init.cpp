...
int SecondStageMain(int argc, char** argv) {
    ...
    // --- OFOX STUB HANDOFF ---
    // If recovery-init-stub already unpacked the cluster (/system/bin/fox.init
    ...
    // overwrite the RUNNING fox.init and its mapped libs (ETXTBSY) and fail
    // the verify below. Legacy path stays for stub-less boots only.
    // NOTE: fox.init (not init.real) avoids collisions with Magisk/KSU chains.
    bool ofox_stub_done = (access("/system/bin/fox.init", F_OK) == 0);
    if (ofox_stub_done) {
