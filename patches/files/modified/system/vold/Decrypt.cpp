	...
	std::string unwrapSyntheticPasswordBlob(const std::string& spblob_path, const std::string& handle_str, const userid_t user_id,
		...
		if (*synthetic_password_version == SYNTHETIC_PASSWORD_VERSION_V2
			...
			* https://android.googlesource.com/platform/frameworks/base/+/android-8.0.0_r23/keystore/java/android/security/keystore/AndroidKeyStoreAuthenticatedAESCipherSpi.java#216 */
			...
			if (*synthetic_password_version == SYNTHETIC_PASSWORD_VERSION_V4) {
				// V4 (Android 17): same blob layers as V3 (keystore AES-GCM,
				...
				auto try_key = [&](const char* nm, const std::string& k) -> bool {
					...
					if (fscrypt_unlock_ce_storage(user_id, k)) {
						printf("[OFOX] v4 KDF %s OK\n", nm);
						disk_decryption_secret_key = k;
						return true;
					}
					return false;
				};
				// SP800-108 counter mode, HMAC-SHA512 PRF, L=256 (32 bytes).
				{
					HMAC_CTX ctx;
					HMAC_CTX_init(&ctx);
				...
				// Research fallback: KDF playground via fox_fbe_kdf driven
				// by /tmp/fox_kdf.conf (adb-pushable, no rebuilds), then
				// /system/etc/fox_kdf.conf.
				{
					...
					for (int ci = 0; ci < 2 && exprs.empty(); ci++) {
						...
						fclose(f);
						if (!exprs.empty())
							printf("[OFOX] v4 playground %s (%zu exprs)\n", confs[ci], exprs.size());
					}
					for (auto& e : exprs) {
