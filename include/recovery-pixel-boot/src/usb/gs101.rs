...
/// gs101 (Tensor G1) is a 6.1-only family: the 6.1 glue owns host mode via
/// the `host_ready && host_on` gate and has no `usb_role_switch` consumer,
/// so host goes through the shim path. A >= 6.12 kernel here is unverified
/// (role-switch store semantics unknown on this SoC) → fail closed.
...
pub fn run_otg_patch() -> Result<(), String> {
    ...
    if kernel_unsupported() {
        info("kernel 6.12+: gs101 host path UNVERIFIED on this kernel, staying in device mode");
        let _ = set_prop("sys.usb.patch_dwc3", "0");
        return Err("gs101 otg: unsupported kernel (needs 6.1 Samsung gate)".into());
    }
    ...
    // 2. Host enable: shim + host_ready (replaces the AOC probe, absent in
    // recovery — aoc_usb_dev.c:385 is the only stock producer).
    if !load_shim() {
        let _ = set_prop("sys.usb.patch_dwc3", "0");
