...
| `otg-patch` | `otg_enable` service | Inject `otg_host_shim` (`.ko` scoring), `patch_dwc3=1`, max77759 switch over I2C |
| `otg-auto` | `patch_dwc3=1` trigger | VBUS host/device daemon, always resident |
...
| `boot.rs` | `boot` stage: preload, firmware, haptics, magisk |
| `otg.rs` | OTG patch and VBUS arbitration (gate: without a PC — force host; debounced polling + settle; UDC rebind in device mode) |
