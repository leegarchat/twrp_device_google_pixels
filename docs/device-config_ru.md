...
- `wide_theme` (дефолт пусто = базовая `/twres`): широкие слабы называют
  свой предсгенерированный XML-вариант (например `"twres_1440"` для
  панелей 1440; см. `tools/theme_wide.py`), штампуется как
  `ro.recovery.theme`, чтобы динамический выбор темы TWRP (gui.cpp)
  грузил подходящие страницы. В совпавшей теме все три скейлера
  сходятся (~1.0), и текст/картинки/боксы не разъезжаются. Планшет так
  же использует `twres_1600` (натив на весь экран, без леттербокса).
- `inner_theme` (дефолт пусто): то же для внутреннего канваса фолда,
  выбирается по hinge на старте вместе с `inner_display` (нативные
  размеры панели, `inner_progressive_scale` 0). Закрытые фолды берут
  `wide_theme` (обычно пусто). Нюанс: hinge читается один раз на
  early-init — складывание посреди сессии канвас и тему не меняет
  (старое ограничение).
